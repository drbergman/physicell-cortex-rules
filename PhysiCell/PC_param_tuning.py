...
def get_metric(output_folder_name, runs):                         # this function obtains the cell count from a given iteration's output folder
    avgCells = 0
    cell_counts = np.zeros(runs)                                  # initializes array to store cell counts
    for i in range(runs):
        time_waited = 0
        while not os.path.exists(output_folder_name + "-run_" + str(i) + "/final_cells.mat"):    # waits for the final_cells file to be created
            time.sleep(1)
            time_waited += 1
            if time_waited > 30:
                raise Exception("Error: final_cells.mat not found in " + output_folder_name + "-run_" + str(i))

        final_path = output_folder_name + "-run_" + str(i) + "/final_cells.mat"    # ... using the final_cells MatLab file.
        ...
        for layer, id in layers_by_index.items():               
            cell_count += len(output[output[5] == id])
        avgCells += cell_count
        cell_counts[i] = cell_count
    
    print(f"Finished with {output_folder_name}-run_*")
    print(f"\tcell_counts: {cell_counts}")
    print(f"\tmean: {np.mean(cell_counts)}")
    print(f"\tstd: {np.std(cell_counts)}")
    if np.std(cell_counts) == 0:
        print(f"\tsignal-to-noise ratio: inf")
    else:
        print(f"\tsignal-to-noise ratio: {np.mean(cell_counts) / np.std(cell_counts)}")
...
def iterate_param(previous_pair, current_pair):
    ...
    if abs(step) < min_step:
        step = min_step * np.random.choice([-1, 1])
...
def modify_slurm(startIndex, folder_name):
    ...
    with open(f"{physicell_dir}/parallel_runs_drb.slurm", "r") as file:
        lines = file.readlines()
        lines[1] = lines[1][:29] + str(startIndex) + "-" + str(startIndex + runs_per_set-1) + "\n"
        lines[14] = lines[14][:12] + str("'" + folder_name + "'\n")
        lines[15] = lines[15][:6] + str(startIndex) + "\n"
        lines[16] = lines[16][:4] + str(startIndex + runs_per_set) + "\n"

    with open(f"{physicell_dir}/parallel_runs_drb.slurm", "w") as file:
...
def run_sim(param_value):
    ...
    output_folder_name = output_prefix + parameter_name + "_" + str(param_value)    # output folder format is './{output_prefix}-{parameter_name}={param_value}'
    ...
    if parameter_location == "settings":
        ...
        raise Exception("parameter location not specified. please set parameter_location to 'rules' or 'settings'.")


    # runs model
    os.system('rm -rf ./' + output_folder_name + "-run_*")
    ...
    for i in startVals:
        # waiting_for_previous_to_run = True
        # while waiting_for_previous_to_run:
        #     job_qs = [subprocess.check_output(["squeue", "-j", job_id.decode().rstrip('\n')]) for job_id in job_ids]
        #     print("job_qs:", job_qs)    
        #     continue_waiting = False
        #     for job_q in job_qs:
        #         if " R " not in job_q.decode():
        #             continue_waiting = True
        #             time.sleep(1)
        #             break # leave for loop
        #     waiting_for_previous_to_run = continue_waiting

        queue = subprocess.check_output(["squeue", "-u", user_name])

        while "PD" in queue.decode():
            time.sleep(0.1)
            queue = subprocess.check_output(["squeue", "-u", user_name])
            print(f"Waiting for queue to clear: {queue.decode()}")

        modify_slurm(i, output_folder_name)
        job_ids.append(subprocess.check_output(["sbatch", f"{physicell_dir}/parallel_runs_drb.slurm"])[20:28])
        print("Submitting", i, "-", i + runs_per_set-1, ":", job_ids[-1].decode())
    
        time.sleep(1)
    
    queue = subprocess.check_output(["squeue", "-u", user_name])
    job_ids_str = [job_id.decode().rstrip('\n') for job_id in job_ids]

    # check if any job_ids (as strings) are in the queue. If they are, wait 1 second and check again.
    while any([job_id.decode().rstrip('\n') in queue.decode() for job_id in job_ids]):
        time.sleep(1)
        queue = subprocess.check_output(["squeue", "-u", user_name])
    ...
    metric = get_metric(output_folder_name, sets * runs_per_set)

    os.system('rm -rf ./' + output_folder_name + "-run_*")   # deletes output folders after successful runs
    
    return metric

# run_sim(initial_value)
...
while not check_metric(curr[1], target_metric):
    ...
    if np.isnan(new_val):
        raise Exception("NaN value detected. Please check the output of the last two runs.")
...
def multiDistLayersHist(output_prefix: str, parameter_name: str, parameter_values: list, layers: dict):
    for i in parameter_values:
        final_path = output_prefix + parameter_name + "_" + str(i) + "/final_cells.mat"
        ...
        plt.savefig("{physicell_dir}/PC_output_images/" + output_prefix + parameter_name + '_' + str(i) + "_layersDist.jpg")
        plt.show()
        plt.clf()
...
# pV = 6600
# output_folder_name = output_prefix + parameter_name + "_" + str(pV)  
# if parameter_location == "settings":
#         tree = ET.parse(config_settings_path)       # locates and opens PhysiCell settings file
#         # tree.find("save").find("folder").text = output_folder_name  # updates name of output folder
#         '''
...
#     row_index = 18
#     column_index = 5
#     rules.iloc[row_index, column_index] = pV
...
#     modify_slurm(i, output_folder_name)
#     # copy slurm file to one suffixed with i
#     shutil.copy(f"{physicell_dir}/parallel_runs_drb.slurm", f"{physicell_dir}/parallel_runs_drb_{i}.slurm")
#     job_ids.append(subprocess.check_output(["sbatch", f"{physicell_dir}/parallel_runs_drb_{i}.slurm"])[20:28])
#     print("Submitting", i, "-", i + runs_per_set-1, ":", job_ids[-1].decode())
#     # print("Submitting", i, "-", i + runs_per_set-1, ":")
